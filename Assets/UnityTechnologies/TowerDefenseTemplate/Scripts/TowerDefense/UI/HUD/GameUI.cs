...
namespace TowerDefense.UI.HUD
{
	...
	public class GameUI : Singleton<GameUI>
	{
		...
		/// <summary>
		/// Upgrades <see cref="currentSelectedTower" /> modifier, if possible
		/// </summary>
		/// <exception cref="InvalidOperationException">
		/// Throws exception when selecting tower when <see cref="State" /> does not equal <see cref="State.Normal" />
		/// or <see cref="currentSelectedTower" /> is null
		/// </exception>
		public void UpgradeSelectedTowerModifier()
		{
			if (state != State.Normal)
			{
				throw new InvalidOperationException("Trying to upgrade modifier whilst not in Normal state");
			}
			if (currentSelectedTower == null)
			{
				throw new InvalidOperationException("Selected Tower is null");
			}
			if (!currentSelectedTower.canUpgradeModifier)
			{
				return;
			}
			int upgradeCost = currentSelectedTower.GetCostForModifier();
			bool successfulUpgrade = LevelManager.instance.currency.TryPurchase(upgradeCost);
			if (successfulUpgrade)
			{
				currentSelectedTower.UpgradeTowerModifier();
			}
			towerUI.Hide();
			DeselectTower();
		}
